...
class GreenhouseFiller(BaseFiller):
    ...
    async def submit_application(self, page) -> bool:
        ...
        if await submit_btn.count() > 0:
            ...
            try:
                ...
                content = await page.content()
                if "sent a" in content.lower() and "code" in content.lower() and await page.locator("input[id*='code'], input[name*='code']").count() > 0:
                     ...
                     try:
                         ...
                         print("   ⏳ Waiting 15s for email to arrive...")
                         import asyncio
                         await asyncio.sleep(15) 
                         
                         code = mail.get_verification_code(subject_filter="Greenhouse")
                         if code:
                             print(f"   🔢 Found Code: {code}. Filling...")
    ...
    async def _handle_custom_questions(self, page, job: Job, application: Application) -> None:
        ...
        for i in range(count):
            ...
            # Check for Location/City Autocomplete (Prioritize this over Dropdown for "Location")
            if "city" in text_lower or "location" in text_lower:
