...
export default function WeaponShowcase() {
  ...
  return (
    <div className="tech-border bg-[var(--valo-darker)]/80 backdrop-blur-md p-6 relative overflow-hidden group">
      {/* Header */}
      <div className="flex justify-between items-end mb-6 border-b border-white/10 pb-4">
        <div>
           <div className="text-[var(--valo-text-dim)] text-xs tracking-widest mb-1">BUY PHASE</div>
           <h2 className="font-display text-3xl font-bold tracking-wider text-[var(--valo-text)]">
             LOADOUT
           </h2>
        </div>
        <div className="text-right">
             <div className="text-[var(--valo-green)] font-mono text-xl">
      ...
      <div className="flex gap-8">
              ...
              <div key={category} className="space-y-2">
                 ...
                 {WEAPONS.filter(w => w.category === category).map(weapon => (
                    <div 
                      ...
                      className={`
                        relative h-20 border cursor-pointer transition-all duration-200 group/item overflow-visible transform hover:scale-105
                        ${selectedWeapon.id === weapon.id 
                           ? 'bg-[var(--valo-green)]/10 border-[var(--valo-green)]' 
                           : 'bg-[var(--valo-gray)] border-transparent hover:bg-[var(--valo-gray-light)]'
                        }
                      `}
                      style={{
                    ...
                    >
                       <img 
                          src={weapon.icon} 
                          alt={weapon.name}
                          className="absolute right-[-10px] top-1/2 -translate-y-1/2 h-16 object-contain z-10 transition-transform group-hover/item:scale-110 group-hover/item:rotate-3" 
                       />
                       <div className="absolute top-2 left-3 z-20">
                          <div className={`font-display font-bold text-lg leading-none ${selectedWeapon.id === weapon.id ? 'text-[var(--valo-green)]' : 'text-white'}`}>
                             {weapon.name}
                          </div>
        ...
        <div className="w-1/3 bg-[var(--valo-dark)] border border-white/5 p-6 relative flex flex-col items-center text-center">
            {/* Background Mesh */}
            ...
            <div className="flex-1 flex items-center justify-center w-full relative">
                 <img 
                    ...
                    className="w-full object-contain drop-shadow-[0_0_20px_rgba(255,255,255,0.1)] floating-animation"
                 />
            </div>

            <div className="w-full space-y-4 relative z-10 mt-4">
                <div>
                   <h3 className="font-display text-4xl font-bold text-[var(--valo-red)]">{selectedWeapon.name}</h3>
                ...
                <div className="space-y-2">
                   ...
                   <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-[var(--valo-green)] text-sm">{selectedWeapon.xp}</span>
                   </div>
                </div>
                
                <div className="bg-[var(--valo-gray)] p-3 text-xs text-[var(--valo-text)] italic border-l-2 border-[var(--valo-red)] text-left">
                   "{selectedWeapon.desc}"
                </div>

                <button className="w-full py-3 bg-[var(--valo-red)] text-white font-display font-bold tracking-widest hover:brightness-110 active:scale-95 transition-all clip-path-check mt-2">
                   EQUIP
                </button>
            </div>
        </div>
